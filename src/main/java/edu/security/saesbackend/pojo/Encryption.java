...
public class Encryption {
    static int[][]sBox=new int[][]{{0x9,0x4,0xA,0xB},{0xD,0x1,0x8,0x5},{0x6,0x2,0x0,0x3},{0xC,0xE,0xF,0x7}};
    String key0;
    String key1;
    int[] key0Array;
    int[] key1Array;
    ...
    public int[] round1(int[] plainTextArray){
        //        Round 1
        int[] nibble1_round1 = Arrays.copyOfRange(plainTextArray, 0, 4);
        int[] nibble2_round1 = Arrays.copyOfRange(plainTextArray, 4, 8);
        int[] nibble3_round1 = Arrays.copyOfRange(plainTextArray, 8, 12);
        int[] nibble4_round1 = Arrays.copyOfRange(plainTextArray, 12, 16);
        ...
        //log.info("SBOX{}",AesService.BinaryToString(substitutedArray));
        int[] rowShiftedArray= new int[16];
        for(int i=0;i<16;i++){
            if(i>=3&&i<=7){
                rowShiftedArray[i]=substitutedArray[i+8];
            }else if(i>=11&&i<=15){
                rowShiftedArray[i]=substitutedArray[i-8];
            }else{
                rowShiftedArray[i]=substitutedArray[i];
            }
        }
        //log.info("Shift ROWS{}",AesService.BinaryToString(rowShiftedArray));

        int[] nibble1 = Arrays.copyOfRange(rowShiftedArray, 0, 4);
        int[] nibble2 = Arrays.copyOfRange(rowShiftedArray, 4, 8);
        int[] nibble3 = Arrays.copyOfRange(rowShiftedArray, 8, 12);
        int[] nibble4 = Arrays.copyOfRange(rowShiftedArray, 12, 16);
        //log.info("Nibble 1 is {}",nibble1);
        //log.info("Nibble 2 is {}",nibble2);
        //log.info("Nibble 3 is {}",nibble3);
        //log.info("Nibble 4 is {}",nibble4);
        int[] newNibble1=XOR(nibble1,decimalToBinary(GFProduct[4][binaryToDecimal(nibble2)]));
        int[] newNibble2=XOR(nibble2,decimalToBinary(GFProduct[4][binaryToDecimal(nibble1)]));
        int[] newNibble3=XOR(decimalToBinary(GFProduct[4][binaryToDecimal(nibble4)]), nibble3);
        int[] newNibble4=XOR(decimalToBinary(GFProduct[4][binaryToDecimal(nibble3)]),nibble4);
        //log.info("new Nibble1 is {}",newNibble1);
        //log.info("new Nibble2 is {}",newNibble2);
        //log.info("new Nibble3 is {}",newNibble3);
        //log.info("new Nibble4 is {}",newNibble4);
        ...
        int[] plusArray=XOR(mixColArray,subKey1);
        //log.info("add round 1 key {}",AesService.BinaryToString(plusArray));
        return round2(plusArray);
    }
    public int[] round2(int[] plainTextArray){
        ...
        //log.info("shift rows {}",AesService.BinaryToString(rowShiftedArray));
        int[] plusArray=XOR(subKey2,rowShiftedArray);
        //log.info("add round 2 key {}",AesService.BinaryToString(plusArray));
        return plusArray;
    }

    public int[] encrypt(Object plainText) {
        int[] plainTextArray=new int[16];
        if (plainText instanceof Character) {
            plainTextArray=encryptChar((char) plainText);
        } else if (plainText instanceof String) {
            plainTextArray=encryptString((String) plainText);
        }
        ...
        //log.info("key0{}",subKey0);
        //log.info("key1{}",subKey1);
        //log.info("key2{}",subKey2);
//        Add round 0 key
        plainTextArray=XOR(plainTextArray,subKey0);
        //log.info("ADD round 0 key{}",AesService.BinaryToString(plainTextArray));
//        round 1
        round1(plainTextArray);
        return round1(plainTextArray);
    }

    public static int[] encryptChar(char plainText) {
    ...
    public static int[] encryptString(String binaryText) {
        int[] binaryArray = new int[16];
        for (int i = 0; i < 16; i++) {
            binaryArray[i] = binaryText.charAt(i) - '0';
        }
